...
int32_t StringBuffer::indexOf(const char *look_for, size_t look_for_length, int32_t start_index)
{
    const char *buffer = data == NULL ? immutable : data;
    assert(buffer != NULL);
    ...
    for (size_t pos = start_index; pos < length; pos++)
    {
        ...
        if (buffer[pos] == *look_for)
        {
            ...
            {
                if (buffer[pos + sub] != look_for[sub])
                    break;
            }
...
void StringBuffer::setLength(unsigned int l)
{
    ASSERT_OR_FAIL_FAST(data != NULL);
    length = l;
    data[l] = char(0);
